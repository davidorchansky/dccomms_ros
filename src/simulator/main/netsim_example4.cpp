...
int main(int argc, char **argv) {
  ...
  std::string packetLib = "/home/centelld/programming/dccomms_examples/modules/dccomms_packets/build/libdccomms_packets_d815bac.so";
  std::string pbName = "DataLinkFrameBuilderCRC16";

  sim->SetPacketBuilder("rov", PACKET_TYPE::TX_PACKET, packetLib, pbName);
  sim->SetPacketBuilder("rov", PACKET_TYPE::RX_PACKET, packetLib, pbName);

  sim->SetPacketBuilder("operator", PACKET_TYPE::TX_PACKET,
                        packetLib, pbName);
  sim->SetPacketBuilder("operator", PACKET_TYPE::RX_PACKET, packetLib, pbName);
